...
<table width="100%" cellpadding="10">
    ...
    <tr>
        <?php if ( $payment['target'] === BooklyLib\Entities\Payment::TARGET_APPOINTMENTS ) : ?>
            ...
            <th style="border: 2px solid #eee;text-align:left;"><b><?php esc_html_e( 'Account Number', 'bookly' ) ?></b></th>
    ...
    <?php foreach ( $payments as $paymentsInfo ) : ?>
         <?php
            ...
            $paymentData = $paymentsInfo->getPaymentData();
            $masterInvoiceInfo = $paymentData['payment']['masterInvoiceInfo'];
        ...
        <?php foreach ( $masterInvoiceInfo['items'] as $item ) : ?>
            ...
            <tr>
                ...
                <td style="border-top: 2px solid #eee; border-left: 2px solid #eee;border-right: 2px solid #eee;text-align:left;">
                    <?php if ( $item['customer_fields'] ) echo $ac_number ?>
                </td>
    ...
    <?php endforeach ?>
    <tr>
        <td style="border: 2px solid #eee;text-align:right;" colspan="4"><?php esc_html_e( 'Subtotal', 'bookly' ) ?></td>
        <?php if ( $show['deposit'] ) : ?>
    ...
    <?php foreach ( $adjustments as $adjustment ) : ?>
        <tr>
            <td style="border: 2px solid #eee;text-align:right;" colspan="4"><?php echo esc_html( $adjustment['reason'] ) ?></td>
    ...
    <?php if ( $show['price_correction'] && (float) $masterInvoiceInfo['price_correction'] ) : ?>
        <tr>
            <td style="border: 2px solid #eee;text-align:right;" colspan="4"><?php echo \Bookly\Lib\Entities\Payment::typeToString( $masterInvoiceInfo['type'] ) ?></td>
    ...
    <?php endif ?>
    <tr>
        <td style="border: 2px solid #eee;text-align:right;" colspan="4>"><b><?php esc_html_e( 'Total', 'bookly' ) ?></b></td>
    ...
    <?php if ( $masterInvoiceInfoData['grandTotal'] != $masterInvoiceInfo['paid'] ) : ?>
        <tr>
            <td style="border: 2px solid #eee;text-align:right;" colspan="4>"><b><?php esc_html_e( 'Paid', 'bookly' ) ?></b></td>
        ...
        <?php if ( ( $masterInvoiceInfoData['grandTotal'] - $masterInvoiceInfo['paid'] ) > 0 || ( $show['taxes'] && ( $masterInvoiceInfo['tax_total'] - $masterInvoiceInfo['tax_paid'] ) > 0 ) ) : ?>
            <tr>
                <td style="border: 2px solid #eee;text-align:right;" colspan="4>"><b><?php esc_html_e( 'Due', 'bookly' ) ?></b></td>
