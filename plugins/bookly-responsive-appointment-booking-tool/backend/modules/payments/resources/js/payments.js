...
jQuery(function($) {
    ...
    $('.bookly-generate-js-select').booklySelect2({
        width: '100%',
        allowClear: true,
        placeholder: '',
        theme: 'bootstrap4',
    ...
    $master_invoice.on('click', function () {
        if($customer_filter.val() === null && $category_filter.val() !== "" && $payments_list.find('tbody input:checked').length>=1){
            ...
            alert("Please select more than 1 checkboxes to generate master invoice by category");
        }
    });
});
...
jQuery(function($) {
    ...
    $('.bookly-master-js-select').booklySelect2({
        width: '100%',
        allowClear: true,
        placeholder: '',
        theme: 'bootstrap4',
