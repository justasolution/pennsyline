...
class Ajax extends Lib\Base\Ajax
{
    ...
    public static function getPayments()
    {
        ...
        $query = Lib\Entities\Payment::query( 'p' )
            ->select( 'p.id, p.created_at, p.type, p.paid, p.total, p.status, p.details, c.full_name customer, st.full_name provider,ct.name AS category_name, s.title service, a.start_date' )
        ...
        if ( $filter['category'] != '' ) {
            $query->where( 'ct.id', $filter['category'] );
        }
        ...
        foreach ( $payments as $payment ) {
            ...
            $data[] = array(
                ...
                'category'   => ( $details['items'][0]['service_category'] ),
                'mady_extra'      => ( $payment),
                'mady_extra_1'      => ( $details),
    ...
    /**
         * Get payments. //TODO big logic
         */
    public static function getGayments()
    {
        $filter  = self::parameter( 'filter' );
        ...
        "<pre>".print_r(json_encode($payments),true)."</pre>";
    ...
    /**
             * Get payments. //TODO big logic
             */
    public static function getMayments()
    {
        $filter  = self::parameter( 'filter' );
