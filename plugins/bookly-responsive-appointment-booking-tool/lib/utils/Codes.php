...
abstract class Codes
{
    ...
    public static function getAppointmentCodes( Entities\Appointment $appointment, $format = 'text' )
    {
        ...
        foreach ( $appointment->getCustomerAppointments( true ) as $customer_appointment ) {
            //echo "<pre>".print_r($customer_appointment -> getFields(),true)."</pre>";
    ...
    public static function getCustomerAppointmentCodes( Entities\CustomerAppointment $customer_appointment, $format = 'text' )
    {
        ...
        $customer = Lib\Entities\Customer::find( $customer_appointment->getCustomerId() );
        ...
        $codes = array(
            ...
            'appointment_token' => $customer_appointment->getToken(),
            'approve_appointment_url'  => $customer_appointment->getToken() ? admin_url( 'admin-ajax.php?action=bookly_approve_appointment&token=' . urlencode( Lib\Utils\Common::xorEncrypt( $customer_appointment->getToken(), 'approve' ) ) ) : '',
